...
const shaders = Shaders.create({
  helloBlue: {
    ...
    uniform float red;
    void main() {
      gl_FragColor = vec4(blue, uv.y, red, 0.85);
    }`
  },
  zebraWarp: {
    frag: GLSL`
  ...
  gradientSky:{
    ...
    #define DENS 4.          // tau.rho at the center
    #define rad 0.259     	 // sphere radius

    #define ANIM true        // true/false
    #define PI 3.14159

    vec3 skyColor = vec3(0.25,.428,1.0);
    vec3 sunColor = vec3(0.8,.129,314.57);   // Energy
    ...
    float noise( in vec3 x ) {
      ...
      float n = p.x + p.y*19.0 + 113.0*p.z;
    ...
    vec3 fbm3( vec3 p )
    {
      if (ANIM) p += iGlobalTime/25.;
        float fx = fbm(p);
        float fy = fbm(p+vec3(2345.67,0,45.67));
    ...
    vec3  sphericalTransmittanceGradient(vec2 L, float r, float h, float z)
    {
      ...
      float c1 = S*xmax3+5.*Lx2*r2*Y2*Ly2-3.*R2*Y2*Ly2+3.*H2*Y2*Ly2-5.*Lx2*Y4*Ly2
        ...
        -21.*S*a12*Lx2*Y*Ly+15.*S*Ly3*a12*Lx2*Y+12.*S*Lx4*a12*Y*Ly
    ...
    void main() {
      ...
      //camera
      vec3 cameraPos = vec3(0.2,1.23,0.8);
      vec3 cameraTarget = vec3(0.0, -0.5, 0.15);
      ...
      //light
      vec2 iM = vec2(iResolution.x*0.7, iResolution.y*0.27);

      float theta = (iM.x / iResolution.x *2.25 - 1.)*PI;
